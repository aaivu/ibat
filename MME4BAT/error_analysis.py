...
for segment, group in pred_run.groupby("segment"):
    for row_index, row in group.iterrows():
        ...
        pred_run.at[row_index, "med_convlstm"] = med_convlstm[str(float(segment))].loc[
            date_time
        ]
...
rmse = np.sqrt(
    mean_squared_error(pred_run["run_time_in_seconds"], pred_run["med_convlstm"])
)

print("RMSE (1): %f" % (rmse))

mape = mean_absolute_percentage_error(
    pred_run["run_time_in_seconds"], pred_run["med_convlstm"]
)
print("MAPE (1): %f" % (mape))
...
for segment, group in pred_run.groupby("segment"):
    for row_index, row in group.iterrows():
        date_time = row["DateTimeRef"]
        pred_run.at[row_index, "out_convlstm"] = out_convlstm[str(float(segment))].loc[
            date_time
        ]
...
path_convlstm_out = (
    "/content/drive/Shareddrives/MSc - Shiveswarran/Predicted values/ES_convlstm.csv"
)
...
for segment, group in pred_run.groupby("segment"):
    for row_index, row in group.iterrows():
        date_time = row["DateTimeRef"]
        pred_run.at[row_index, "es_convlstm"] = es_convlstm[str(float(segment))].loc[
            date_time
        ]
...
rmse = np.sqrt(
    mean_squared_error(pred_run["run_time_in_seconds"], pred_run["es_convlstm"])
)

print("RMSE (1): %f" % (rmse))

mape = mean_absolute_percentage_error(
    pred_run["run_time_in_seconds"], pred_run["es_convlstm"]
)
print("MAPE (1): %f" % (mape))
...
for segment, group in pred_run.groupby("segment"):
    for row_index, row in group.iterrows():
        date_time = row["DateTimeRef"]
        pred_run.at[row_index, "ES_convlstm_bfill"] = ES_convlstm_bfill[
            str(float(segment))
        ].loc[date_time]
...
rmse = np.sqrt(
    mean_squared_error(pred_run["run_time_in_seconds"], pred_run["ES_convlstm_bfill"])
)

print("RMSE (1): %f" % (rmse))

mape = mean_absolute_percentage_error(
    pred_run["run_time_in_seconds"], pred_run["ES_convlstm_bfill"]
)
print("MAPE (1): %f" % (mape))
...
for segment, group in pred_run.groupby("segment"):
    for row_index, row in group.iterrows():
        date_time = row["DateTimeRef"]
        pred_run.at[row_index, "ES_purelstm_med"] = ES_purelstm_med[
            str(float(segment))
        ].loc[date_time]
...
rmse = np.sqrt(
    mean_squared_error(pred_run["run_time_in_seconds"], pred_run["ES_purelstm_med"])
)

print("RMSE (1): %f" % (rmse))

mape = mean_absolute_percentage_error(
    pred_run["run_time_in_seconds"], pred_run["ES_purelstm_med"]
)
print("MAPE (1): %f" % (mape))
...
path_convlstm_out = (
    "/content/drive/Shareddrives/MSc - Shiveswarran/Predicted values/purelstm_med.csv"
)
...
for segment, group in pred_run.groupby("segment"):
    for row_index, row in group.iterrows():
        date_time = row["DateTimeRef"]
        pred_run.at[row_index, "purelstm_med"] = purelstm_med[str(float(segment))].loc[
            date_time
        ]
...
rmse = np.sqrt(
    mean_squared_error(pred_run["run_time_in_seconds"], pred_run["purelstm_med"])
)

print("RMSE (1): %f" % (rmse))

mape = mean_absolute_percentage_error(
    pred_run["run_time_in_seconds"], pred_run["purelstm_med"]
)
print("MAPE (1): %f" % (mape))
...
pred_run["DowTimeRef"] = pd.to_datetime(
    (pred_run["DayOfWeek"].values) * 24 * 60 * 60 + ix.hour * 60 * 60 + ix.minute * 60,
    unit="s",
)
...
data_melted = pd.melt(
    pred_run,
    id_vars=["time_of_day"],
    value_vars=["XGBoost", "convlstm", "run_time_in_seconds"],
    var_name="Models",
    value_name="Predicted run times",
)
