...
rmse = cross_val_score(
    xgb, X, y, scoring="neg_mean_squared_error", cv=KFold(n_splits=10, shuffle=False)
)
mae = cross_val_score(
    xgb, X, y, scoring="neg_mean_absolute_error", cv=KFold(n_splits=10, shuffle=False)
)
...
xgb = xgb.XGBRegressor(
    colsample_bytree=0.3, learning_rate=0.1, max_depth=5, alpha=10, n_estimators=1000
)
...
scores = cross_val_score(
    lr, X, y, scoring="r2", cv=KFold(n_splits=10, shuffle=False)
)  # shuffle=False
rmse = cross_val_score(
    lr, X, y, scoring="neg_mean_squared_error", cv=KFold(n_splits=10, shuffle=False)
)  #
rmse = np.sqrt(list(-rmse))
lr_r2 = scores.mean()
lr_rmse = rmse.mean()
...
scores = cross_val_score(
    rfr, X, y, scoring="r2", cv=KFold(n_splits=10, shuffle=False)
)  # ,
rmse = cross_val_score(
    rfr, X, y, scoring="neg_mean_squared_error", cv=KFold(n_splits=10, shuffle=False)
)  # ,
rmse = np.sqrt(list(-rmse))
rfr_r2 = scores.mean()
rfr_rmse = rmse.mean()
...
rmse = cross_val_score(
    dt, X, y, scoring="neg_mean_squared_error", cv=KFold(n_splits=10, shuffle=False)
)
rmse = np.sqrt(list(-rmse))
dt_r2 = scores.mean()
dt_rmse = rmse.mean()
...
run_true = pred_run.pivot_table(
    index="trip_id", columns="segment", values="run_time_in_seconds"
)
...
dwell_true = pred_dwell.pivot_table(
    index="trip_id", columns="bus_stop", values="dwell_time_in_seconds"
)
dwell_true

dwell_true = dwell_true.fillna(0)

dwell_pred = pred_dwell.pivot_table(
    index="trip_id", columns="bus_stop", values="XGBoost"
)
dwell_pred
...
pred_dwell["DateTime"] = pd.to_datetime(
    pred_dwell["date"] + " " + pred_dwell["arrival_time"]
)
ref_freq = "15min"
...
path_lstm = (
    "/content/drive/Shareddrives/MSc - Shiveswarran/Predicted values/purelstm.csv"
)
path_convlstm = (
    "/content/drive/Shareddrives/MSc - Shiveswarran/Predicted values/convlstm.csv"
)
...
mape = mean_absolute_percentage_error(
    pred_run["run_time_in_seconds"], pred_run["covlstm"]
)
print("MAPE (1): %f" % (mape))
...
rmse = np.sqrt(
    mean_squared_error(pred_run["run_time_in_seconds"], pred_run["LightGBM"])
)

print("RMSE (1): %f" % (rmse))

mape = mean_absolute_percentage_error(
    pred_run["run_time_in_seconds"], pred_run["LightGBM"]
)
print("MAPE (1): %f" % (mape))
...
mape = mean_absolute_percentage_error(
    pred_run["run_time_in_seconds"], pred_run["XGBoost"]
)
print("MAPE (1): %f" % (mape))
...
run_true = pred_run_f.pivot_table(
    index="trip_id", columns="segment", values="run_time_in_seconds"
)
run_true = run_true.fillna(50)
...
dwell_true = pred_dwell_f.pivot_table(
    index="trip_id", columns="bus_stop", values="dwell_time_in_seconds"
)
dwell_true = dwell_true.fillna(0)

dwell_pred = pred_dwell_f.pivot_table(
    index="trip_id", columns="bus_stop", values="cnn_lstm"
)
dwell_pred = dwell_pred.fillna(0)
